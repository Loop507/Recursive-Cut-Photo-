...
# --- ENGINE ---
def generate_master(imgs, up_aud, mode, orientation, strand_val, is_random, max_limit, use_base, k_params, op_params):
    w, h = 1280, 720
    fps = 24
    ...
    for f in range(total_f):
        ...
        # 2. Opacità Ricomposizione (Separata)
        top_alpha = 0.0
        if curr_s > op_params['start_fade']:
            t_fade = (curr_s - op_params['start_fade']) / (max_limit - op_params['start_fade'])
            top_alpha = t_fade * (op_params['final_v'] / 100)

        frame = np.zeros((h, w, 3), dtype=np.uint8)
        
        if mode == "Kinetic (Flusso)":
            for s, (start, end) in enumerate(boundaries):
                offsets[s] += 0.01 + (val * 0.5)
                idx, nxt, alpha = int(offsets[s]%len(imgs)), int((offsets[s]+1)%len(imgs)), offsets[s]%1
                strip = cv2.addWeighted(imgs[idx][start:end, :] if orientation=="Orizzontale" else imgs[idx][:, start:end], 1-alpha, 
                                        imgs[nxt][start:end, :] if orientation=="Orizzontale" else imgs[nxt][:, start:end], alpha, 0)
                if orientation=="Orizzontale": frame[start:end, :] = strip
                else: frame[:, start:end] = strip
        else:
            idx_base = (f * val * 10)
            for s, (start, end) in enumerate(boundaries):
...
with st.container():
    st.subheader("📁 1. Risorse")
    c_files1, c_files2 = st.columns(2)
    with c_files1: up_img = st.file_uploader("Carica Immagini", type=["jpg","png","jpeg"], accept_multiple_files=True)
    with c_files2: up_aud = st.file_uploader("Carica Audio (Facoltativo)", type=["mp3","wav"])

st.divider()

# SEZIONE 2: IL TRITATO (KEYFRAMES)
st.subheader("✂️ 2. Misure Effetto Tritato")
ck1, ck2, ck3 = st.columns(3)
with ck1:
    s_t = st.number_input("Inizio Movimento (sec)", 0.0, 60.0, 3.0)
    s_v = st.slider("Potenza Iniziale %", 0, 100, 5)
with ck2:
    p_t = st.number_input("Picco Massimo (sec)", 0.0, 60.0, 5.0)
    p_v = st.slider("Potenza Picco %", 0, 100, 100)
with ck3:
    e_v = st.slider("Potenza Finale %", 0, 100, 3)

st.divider()

# SEZIONE 3: RICOMPOSIZIONE (OPACITÀ)
st.subheader("👁️ 3. Ricomposizione Finale (Opacità)")
co1, co2 = st.columns(2)
with co1:
    fade_start = st.number_input("Inizio Dissolvenza (sec)", 0.0, 60.0, 5.5, help="Da che secondo la foto base inizia a coprire il tritato?")
with co2:
    fade_end_v = st.slider("Opacità Finale %", 0, 100, 100, help="100% = Foto finale pulita. 0% = Resta tutto tritato.")

st.divider()

# SEZIONE 4: STILE & RENDERING
st.subheader("⚙️ 4. Stile & Rendering")
cs1, cs2, cs3 = st.columns(3)
with cs1:
    mode = st.radio("Effetto", ["Kinetic (Flusso)", "Recursive (Stutter)"])
    orientation = st.radio("Taglio", ["Orizzontale", "Verticale"])
with cs2:
    strand_val = st.slider("Linee (px)", 1, 500, 60)
...
if st.button("🎬 GENERA MASTER") and up_img:
    ...
    with st.spinner("Creando l'evoluzione..."):
        imgs = [np.array(Image.open(f).convert("RGB").resize((1280, 720))) for f in up_img[:25]]
        path = generate_master(imgs, up_aud, mode, orientation, strand_val, is_random, max_limit, use_base, k_p, o_p)
        st.video(path)
        with open(path, "rb") as f: st.download_button("💾 SCARICA MASTER", f, file_name="loop507.mp4")
